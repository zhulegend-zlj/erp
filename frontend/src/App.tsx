...
const HOME_GUIDES: Record<Role, string[]> = {
  ...
  sales: [
    '订单：新建订单（客户/客户PO号/订单日期/付款条件；明细每行填 成品/数量/单价/客户交期/ZRH交期，不同成品交期可不同）→ 确认订单；出货完成后推进到「已完成」。客户发订单截图时点「一键导入图片」自动填明细。',
    '编辑订单：同一PO号要加成品/改数量/改交期时点订单行「编辑」（草稿随意改；已确认但未开始采购/生产/出货的也可改；有业务记录即锁定）。',
