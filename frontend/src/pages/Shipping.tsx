...
function LinesEditor({
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 90,
      render: (_: unknown, r: LineRow) => (
        <InputNumber min={1} precision={0} value={r.qty ?? undefined} onChange={(v) => patch(r.key, { qty: v })} style={{ width: '100%' }} />
      ),
    },
    {
      title: '单价',
      ...
      width: 100,
      render: (_: unknown, r: LineRow) => (
        <InputNumber min={0} precision={2} value={r.unitPrice ?? undefined} onChange={(v) => patch(r.key, { unitPrice: v })} style={{ width: '100%' }} />
      ),
    },
    {
      title: '客户PO',
      ...
      width: 110,
      render: (_: unknown, r: LineRow) => (
        <Input value={r.customerPoNo ?? ''} onChange={(e) => patch(r.key, { customerPoNo: e.target.value })} />
      ),
    },
    {
      title: 'Lot',
      ...
      width: 110,
      render: (_: unknown, r: LineRow) => (
        <Input value={r.lotNo ?? ''} onChange={(e) => patch(r.key, { lotNo: e.target.value })} />
      ),
    },
    {
      title: '箱数',
      ...
      width: 80,
      render: (_: unknown, r: LineRow) => (
        <InputNumber min={0} precision={0} value={num(r.cartons)} onChange={(v) => patch(r.key, { cartons: v })} style={{ width: '100%' }} />
      ),
    },
    {
      title: '净重(kg)',
      ...
      width: 90,
      render: (_: unknown, r: LineRow) => (
        <InputNumber min={0} precision={3} value={num(r.netWeight)} onChange={(v) => patch(r.key, { netWeight: v })} style={{ width: '100%' }} />
      ),
    },
    {
      title: '毛重(kg)',
      ...
      width: 90,
      render: (_: unknown, r: LineRow) => (
        <InputNumber min={0} precision={3} value={num(r.grossWeight)} onChange={(v) => patch(r.key, { grossWeight: v })} style={{ width: '100%' }} />
      ),
    },
    {
      title: 'CBM',
      ...
      width: 90,
      render: (_: unknown, r: LineRow) => (
        <InputNumber min={0} precision={4} value={num(r.cbm)} onChange={(v) => patch(r.key, { cbm: v })} style={{ width: '100%' }} />
      ),
    },
    {
      title: '柜号',
      ...
      width: 130,
      render: (_: unknown, r: LineRow) => (
        <Input value={r.containerNo ?? ''} onChange={(e) => patch(r.key, { containerNo: e.target.value })} />
      ),
    },
    {
      title: '封条',
      ...
      width: 110,
      render: (_: unknown, r: LineRow) => (
        <Input value={r.sealNo ?? ''} onChange={(e) => patch(r.key, { sealNo: e.target.value })} />
      ),
    },
    {
      title: 'HBL',
      ...
      width: 120,
      render: (_: unknown, r: LineRow) => (
        <Input value={r.hblNo ?? ''} onChange={(e) => patch(r.key, { hblNo: e.target.value })} />
      ),
    },
    {
      title: '备注',
      ...
      width: 110,
      render: (_: unknown, r: LineRow) => (
        <Input value={r.remark ?? ''} onChange={(e) => patch(r.key, { remark: e.target.value })} />
      ),
    },
    {
      title: '操作',
...
export default function Shipping() {
  ...
  async function handleShip(values: Record<string, string | number>) {
    if (!values.salesOrderId) {
      message.warning('请选择待出货订单')
      return
    }
    ...
    const totalByOrder = (selectedOrder?.items ?? []).reduce((s, it) => s + it.qty, 0)
    const totalByLines = lines.reduce((s, l) => s + Number(l.qty ?? 0), 0)
    if (totalByLines !== totalByOrder) {
      message.warning('明细行数量合计与订单不符（订单 ' + totalByOrder + ' 台，当前 ' + totalByLines + ' 台）')
      return
    }
    setShipping(true)
    try {
  ...
  return (
    <div>
      <Card title="订单出货（单证中心）" style={{ marginBottom: 16 }}>
        {canOperate ? (
          <>
            <Form form={shipForm} layout="vertical" onFinish={handleShip} style={{ marginBottom: 8 }}>
              <div style={META_GRID}>
                <Form.Item name="salesOrderId" label="待出货订单" rules={[{ required: true, message: '选择订单' }]}>
                  <Select
                    placeholder="选择待出货订单"
                    showSearch
                    optionFilterProp="label"
                ...
                <Form.Item name="deliveryNote" label="送货单号">
                  <Input placeholder="送货单号" />
                </Form.Item>
                <Form.Item name="signer" label="签收人">
                  <Input placeholder="签收人" />
                </Form.Item>
                <Form.Item name="invoiceNo" label="发票号">
                  <Input placeholder="如 ZRH20260814006" />
                </Form.Item>
                <Form.Item name="paymentTerms" label="付款条件">
                  <Input placeholder="如 NET 60" />
                </Form.Item>
                <Form.Item name="incoterm" label="贸易条款">
                  <Select allowClear placeholder="如 FCA" options={INCOTERM_OPTIONS} />
                </Form.Item>
                <Form.Item name="mark" label="唛头">
                  <Input placeholder="如 FANATEC" />
                </Form.Item>
                <Form.Item name="origin" label="原产地" initialValue="China">
                  <Input placeholder="China" />
                </Form.Item>
                <Form.Item name="hsCode" label="海关编码">
                  <Input placeholder="如 9504 50 0000" />
                </Form.Item>
                <Form.Item name="taxRate" label="税率" initialValue="0">
                  <Input placeholder="0" />
                </Form.Item>
                <Form.Item name="vesselVoyage" label="船名/航次">
                  <Input placeholder="如 CMA CGM ZHENG HE / 0FMMMW1MA" />
                </Form.Item>
                <Form.Item name="etd" label="ETD（预计开船）">
                  <Input type="date" />
                </Form.Item>
                <Form.Item name="eta" label="ETA（预计到港）">
                  <Input type="date" />
                </Form.Item>
                <Form.Item name="shippingInstructions" label="运费说明">
                  <Input placeholder="如 ALU 1264 pcs" />
                </Form.Item>
            ...
            {selectedOrder ? (
              <p style={{ marginBottom: 8 }}>
                ...
                {(selectedOrder.items ?? []).map((it) => it.product.name + '×' + it.qty).join('、')}）——
                明细行数量合计须与订单一致，同一成品可点「拆行」按柜/批次拆分
              </p>
            ) : null}
