...
const CFG: ProductCfg[] = [
  ...
  { file: 'MPM/RM-CS MPM RFCL-BOM清单-最新版.xlsx', productSku: 'RM_CS-MPM-JLD', productName: 'CS MPM 拨片（JLD 出货·不锁碳纤板）', productNameEn: 'CS MPM JLD', miscPrefix: 'CSMPM-', renameIds: { 'P1806-10928': 'P1806-10928-MPM' }, nameOverride: { 'M3 x 12 平头内六角螺丝': 'ESTP-11186' } },
  { file: 'MPM/RM-CS-MPM-BOM清单JLD.xlsx', productSku: 'RM_CS-MPM-JLD-简', productName: 'CS MPM 拨片（JLD 出货·减配）', productNameEn: 'CS MPM JLD', miscPrefix: 'CSMPM-', renameIds: { 'P1806-10928': 'P1806-10928-MPM' } },
  { file: 'APM/RM-P APM 出货JLD-BOM.xlsx', productSku: 'RM_P_APM-JLD金', productName: 'P APM 拨片（JLD 出货·金色磁铁）', productNameEn: 'P APM JLD', miscPrefix: 'PAPM-', magnetColor: '金' },
  { file: 'APM/RM-P APM BLK 黑色磁铁JLD-BOM.xlsx', productSku: 'RM_P_APM-JLD黑', productName: 'P APM 拨片（JLD 出货·黑色磁铁）', productNameEn: 'P APM JLD BLK', miscPrefix: 'PAPM-', magnetColor: '黑' },
  { file: 'APM/RM-P APM BLK-P1705 EVS-BOM.xlsx', productSku: 'RM_P_APM-EVS黑', productName: 'P APM 拨片（EVS 出货·黑色磁铁）', productNameEn: 'P APM EVS', miscPrefix: 'PAPM-', magnetColor: '黑' },
]
...
function finishTag(finish: string): string {
  if (/黑锌/.test(finish)) return '黑锌'
  if (/白锌/.test(finish)) return '白锌'
  if (/黑镍|黑色镀镍|电镀黑镍/.test(finish)) return '黑镍'
  if (/白镍|电镀白镍/.test(finish)) return '白镍'
  if (/彩锌/.test(finish)) return '彩锌'
  if (/蓝胶/.test(finish)) return '蓝胶'
  if (/本色/.test(finish)) return '本色'
  if (/喷粉黑|电泳黑|黑色阳极|阳极黑/.test(finish)) return '黑'
  if (/白/.test(finish)) return '白'
  return '其他'
}
...
async function buildProduct(cfg: ProductCfg, file: string): Promise<OutRow[]> {
  ...
  for (const r of rows) {
    ...
    if (r.id) {
      ...
      // 标准件按规格+类型命名；表面处理按颜色/镀层比较，不同则加后缀拆分（老板口径 2026-08-31）
      const tag = finishTag(r.finish)
      const existing = await prisma.part.findUnique({ where: { sku: stdSku } })
      if (existing) {
        const existTag = finishTag(existing.finish ?? '')
        if (!r.finish || !existing.finish || existTag === tag) {
          sku = stdSku
        ...
        // 库里没有原规格号：先找已拆分的同规格同表面处理件，再按名称兜底（如 RFCL 的 M3x7 → ESTP-9096）
        if (r.finish) {
          const candPart = await prisma.part.findFirst({ where: { sku: { startsWith: stdSku + '-' }, finish: { contains: tag } } })
          if (candPart) { sku = candPart.sku; note.push('共用已拆分的 ' + candPart.sku) }
        }
        if (!sku) {
          const dbByName =
