...
const parts = await p.part.findMany({ select: { id: true, sku: true, name: true, spec: true, finish: true, price: true, priceInclTax: true, supplierId: true } });
...
const okSheet = XLSX.utils.json_to_sheet(finalOk.map(o => ({
  表页: o.sheet, 行: o.r, 序号: o.no, PO料号: o.skuRaw, 品名规格: o.name, 用量: o.qty,
  新不含税: o.price, 新含税: o.priceIncl, 税点: o.tax, 供应商代码: o.code8,
  对应ERP零件: o.sku, ERP零件名: o.partName, 现不含税: o.oldPrice, 现含税: o.oldPriceIncl, 现供应商: o.oldSup,
  新供应商: o.newSup, 供应商依据: o.supConf,
})));
XLSX.utils.book_append_sheet(wbOut, okSheet, '可导入');
const cfSheet = XLSX.utils.json_to_sheet(conflicts.map(o => ({
  表页: o.sheet, 行: o.r, PO料号: o.skuRaw, 品名规格: o.name, 用量: o.qty, 新不含税: o.price, 新含税: o.priceIncl, 供应商代码: o.code8,
  对应ERP零件: o.sku, ERP零件名: o.partName, 现不含税: o.oldPrice, 现含税: o.oldPriceIncl,
})));
XLSX.utils.book_append_sheet(wbOut, cfSheet, '冲突待定');
const multiSheet = XLSX.utils.json_to_sheet(multiList.map(o => ({
  表页: o.sheet, 行: o.r, PO料号: o.skuRaw, 品名规格: o.name, 用量: o.qty, 新不含税: o.price, 新含税: o.priceIncl,
  候选ERP零件: o.skus.join(' / '), 说明: o.why,
})));
XLSX.utils.book_append_sheet(wbOut, multiSheet, '组合价待定');
const noneSheet = XLSX.utils.json_to_sheet(noneList.map(o => ({
  表页: o.sheet, 行: o.r, PO料号: o.skuRaw, 品名规格: o.name, 用量: o.qty, 新不含税: o.price, 新含税: o.priceIncl, 说明: o.why,
})));
XLSX.utils.book_append_sheet(wbOut, noneSheet, '无对应零件');
const codeSheet = XLSX.utils.json_to_sheet([...codeMap.entries()].sort().map(([code, v]) => ({
  代码: code, 对应供应商: v.supId ? (supName.get(v.supId) || '') : '未知', 依据: v.conf,
})));
XLSX.utils.book_append_sheet(wbOut, codeSheet, '供应商代码对照');
const outPath = 'D:/AI/采购/产品成本单价导入预览-20260901.xlsx';
XLSX.writeFile(wbOut, outPath);

console.log('rows with price:', rows.length);
console.log('可导入 (唯一零件):', finalOk.length);
console.log('冲突待定:', conflicts.length);
for (const c of conflicts) console.log('  冲突: ' + c.sku + ' | ' + c.name + ' | ' + c.price + ' (' + c.sheet + ' r' + c.r + ')');
console.log('组合价待定:', multiList.length);
for (const m of multiList) console.log('  组合: ' + m.skuRaw + ' -> ' + m.skus.join('/') + ' ' + m.why + ' 价=' + (m.price ?? m.priceIncl));
console.log('无对应零件:', noneList.length);
for (const n of noneList) console.log('  无: ' + n.skuRaw + ' | ' + n.name + ' | ' + (n.price ?? n.priceIncl) + ' | ' + n.why);
const withOldPrice = finalOk.filter(o => o.oldPrice != null || o.oldPriceIncl != null).length;
const noOldPrice = finalOk.length - withOldPrice;
console.log('可导入中已有价格:', withOldPrice, ' 无价格(补空):', noOldPrice);
console.log('其中价格将变化:', finalOk.filter(o => o.oldPrice != null && Math.abs(o.oldPrice - (o.price ?? 0)) > 0.0001).length);
console.log('其中含税价将变化:', finalOk.filter(o => o.oldPriceIncl != null && o.priceIncl != null && Math.abs(o.oldPriceIncl - o.priceIncl) > 0.0001).length);
console.log('其中含税价将从空补上:', finalOk.filter(o => o.oldPriceIncl == null && o.priceIncl != null).length);
console.log('无变化(价格含税都相同):', finalOk.filter(o => o.oldPrice != null && Math.abs(o.oldPrice - (o.price ?? 0)) <= 0.0001 && o.oldPriceIncl != null && o.priceIncl != null && Math.abs(o.oldPriceIncl - o.priceIncl) <= 0.0001).length);
