...
- **CSP_V3 已批量导入（2026-08-22，命名规则最终版）**：成品 `CSP-V3`（CSP V3 挂档器）+ 107 个零件 + 107 条 BOM（脚本 `prisma/import-csp-v3.ts`，源表：微信文件目录 CSP_V3清单_物料明细.xlsx）。**命名规则**：①客户官方料号照抄（CSP-003 等）；②CSP-013 七长度变体 = CSP-013-1~7；③无料号自命名零件 = CSP-201 起顺序编号（CSP-2 开头=自命名段）；④螺丝/螺母/垫片等标准件 = 规格+类型（M3x13-杯头、M6-垫片），跨机种同规格共用同一料号，同 SKU 多行用量自动累加。每次导入生成对照表 `D:\AI\erp-backups\CSP-V3-SKU对照表.xlsx`。待复核：序号9「铝套管20*77.8」用量空按 1。图片/图档尚未上传（表内无内嵌图，等工程打包或系统内上传）；供应商/价格由采购后续挂。
