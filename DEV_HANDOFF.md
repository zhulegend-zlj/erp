...
- 登录/改密，中文账号
- 首页分角色使用说明
- 基础资料：客户/供应商/成品/零件/BOM；零件列表按清单表格口径展示（料号/图片/英文品名/中文名称/重量/版本/材质/尺寸规格/表面处理/图档/模具/起订量/价格/图号/供应商），图档支持上传 pdf/dwg/dxf/step/stp/igs/zip/xlsx 等（≤20MB）；新增「工程」角色分工——工程录成品/零件/BOM，采购只挂零件供应商
- 销售订单：新建、状态推进/回退、明细
