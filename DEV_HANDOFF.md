...
- **CSP_V3 已批量导入（2026-08-22）**：成品 `CSP-V3`（CSP V3 挂档器）+ 107 个零件 + 107 条 BOM（脚本 `prisma/import-csp-v3.ts`，源表：微信文件目录 CSP_V3清单_物料明细.xlsx）。命名规则：结构件沿用 CSP-xxx；CSP-013 拆为 CSP-013-长度（7 个）；标准螺丝 ISO 标准号+规格；无料号杂项 中文名[-规格]。待复核：序号9「铝套管20*77.8」用量空按 1。图片/图档尚未上传（表内无内嵌图，等工程打包或系统内上传）；供应商/价格由采购后续挂。
