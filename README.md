...
系统登录账号（中文用户名）：`老板`、`采购`、`仓库`、`销售`、`财务`、`工程`，对应角色分别为 boss / purchase / warehouse / sales / finance / engineer。

分工：**工程**负责录入成品/零件/BOM（含图片与图档，**不填价格**）；**采购**负责维护客户/供应商，并在「零件」页给零件挂供应商、填写价格；其余角色职责不变。

零件图片与「图档地址」支持直接上传文件（图片 + pdf/dwg/dxf/step/stp/igs/zip/xlsx，单个 ≤ 20MB），列表可查看/打开。

**文件目录组织策略**（`backend/uploads/`，已 gitignore）：

```
uploads/
├── <成品SKU>/<零件SKU>-<零件名>/<零件SKU>-<零件名>.png、<零件SKU>-<零件名>-图档.pdf   ← 挂在单个成品下的零件（文件名带零件名）
├── _共用/<零件SKU>-<零件名>/...                      ← 挂在多个成品下的共用零件
└── _未分类/<零件SKU>-<零件名>/...                    ← 尚未挂 BOM（保存 BOM 后自动归位）
成品图片：uploads/<成品SKU>/图片.ext
```

上传时后端自动按零件当前挂载的成品归位；保存 BOM 时自动把 `_未分类` 的文件移到对应成品目录并更新数据库路径；改名/删除零件会同步重命名/删除文件夹。

以下命令用 `backend/prisma/seed.ts` 一次性创建/更新 6 个角色用户（初始密码统一 `88888888`）：
